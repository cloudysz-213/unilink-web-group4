...
export default function EnquiryDetailPage() {
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      if (error) {
        toast.error('Không thể gửi phản hồi: ' + error.message)
      } else {
  ...
  const formatMessage = (msg: any) => {
    ...
    const senderName = isStaff ? 'Academic Staff' : (userProfile?.full_name || 'Student')
  ...
  return (
    <div className="min-h-screen bg-[#f9f9f9]">
            ...
            <div className="bg-primary p-6 rounded-xl text-white flex items-center justify-between overflow-hidden relative shadow-lg">
              ...
              <div className="relative z-10 w-16 h-16">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="material-symbols-outlined text-[#FEB21A] text-xl">timer</span>
                </div>
              </div>
            </div>
          </section>
                    ...
                    <div key={idx} className={`flex gap-4 ${formatted.type === 'staff' ? 'flex-row-reverse' : ''} max-w-[90%] ${formatted.type === 'staff' ? 'ml-auto' : ''}`}>
                      ...
                      <div className={`flex flex-col ${formatted.type === 'staff' ? 'items-end' : 'items-start'} gap-1`}>
                        <div className={`flex items-center gap-2 px-1 ${formatted.type === 'staff' ? 'flex-row-reverse' : ''}`}>
                          <span className="text-[11px] font-bold text-primary uppercase">{formatted.name}</span>
      ...
      {/* Staff Floating Actions Bar - LUÔN HIỂN THỊ 3 BUTTON (không phân biệt role) */}
      {enquiry?.status !== 'resolved' && enquiry?.status !== 'closed' && (
